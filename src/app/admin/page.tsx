...
export default async function AdminPage() {
    const [products, stats, shopName, visitorCount, lowStockThreshold, checkinReward, checkinEnabled, uiBackgroundImage, uiBackgroundBlur] = await Promise.all([
        getProducts(),
        getDashboardStats(),
        (async () => {
            try {
        ...
        (async () => {
            try {
                return await getSetting('ui_background_image')
            } catch {
                return null
            }
        })(),
        (async () => {
            try {
                const v = await getSetting('ui_background_blur')
