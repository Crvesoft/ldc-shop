...
import { deleteProduct, toggleProductStatus, reorderProduct, saveShopName, saveLowStockThreshold, saveCheckinReward, saveCheckinEnabled, saveUiBackgroundImage, saveUiBackgroundBlur } from "@/actions/admin"
...
interface AdminProductsContentProps {
    ...
    uiBackgroundImage: string | null
    uiBackgroundBlur: number
}

export function AdminProductsContent({ products, stats, shopName, visitorCount, lowStockThreshold, checkinReward, checkinEnabled, uiBackgroundImage, uiBackgroundBlur }: AdminProductsContentProps) {
    const { t } = useI18n()
    ...
    const [uiBackgroundImageValue, setUiBackgroundImageValue] = useState(uiBackgroundImage || '')
    const [uiBackgroundBlurValue, setUiBackgroundBlurValue] = useState(String(uiBackgroundBlur || 0))
    const [activeBackgroundImage, setActiveBackgroundImage] = useState(uiBackgroundImage || '')
    const [activeBackgroundBlur, setActiveBackgroundBlur] = useState(uiBackgroundBlur || 0)
    const [savingUiBackgroundImage, setSavingUiBackgroundImage] = useState(false)
    const [savingUiBackgroundBlur, setSavingUiBackgroundBlur] = useState(false)
    ...
    const normalizedBlur = Math.min(100, Math.max(0, Number.parseInt(uiBackgroundBlurValue || '0', 10) || 0))
    const backgroundImageUrl = activeBackgroundImage.trim()
    const backgroundBlurPx = Math.round((activeBackgroundBlur / 100) * 20)
    ...
    const handleSaveUiBackgroundImage = async () => {
        setSavingUiBackgroundImage(true)
        const nextImage = uiBackgroundImageValue.trim()
        try {
            await saveUiBackgroundImage(nextImage)
            setActiveBackgroundImage(nextImage)
            toast.success(t('common.success'))
        } catch (e: any) {
            toast.error(e.message)
        } finally {
            setSavingUiBackgroundImage(false)
        }
    }

    const handleSaveUiBackgroundBlur = async () => {
        setSavingUiBackgroundBlur(true)
        const clampedBlur = Math.min(100, Math.max(0, normalizedBlur))
        try {
            await saveUiBackgroundBlur(String(clampedBlur))
            setUiBackgroundBlurValue(String(clampedBlur))
            setActiveBackgroundBlur(clampedBlur)
            toast.success(t('common.success'))
        } catch (e: any) {
            toast.error(e.message)
        } finally {
            setSavingUiBackgroundBlur(false)
        }
    }
                ...
                <div className="absolute inset-0 -z-10 overflow-hidden">
                    <div
                        ...
                        style={{
                            ...
                            filter: backgroundBlurPx > 0 ? `blur(${backgroundBlurPx}px)` : undefined,
                        }}
                    />
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        ...
                        <div className="border-t pt-4 space-y-3">
                            <div className="grid gap-2 md:max-w-xl">
                                <Label htmlFor="ui-background-image">{t('admin.settings.uiBackgroundImage')}</Label>
                                <Input
                                    id="ui-background-image"
                                    type="url"
                                    value={uiBackgroundImageValue}
                                    onChange={(e) => setUiBackgroundImageValue(e.target.value)}
                                    placeholder={t('admin.settings.uiBackgroundImagePlaceholder')}
                                />
                                <p className="text-xs text-muted-foreground">{t('admin.settings.uiBackgroundImageHint')}</p>
                            </div>
                            <div className="flex flex-wrap items-center gap-3">
                                <Button onClick={handleSaveUiBackgroundImage} disabled={savingUiBackgroundImage}>
                                    {savingUiBackgroundImage ? t('common.processing') : t('admin.settings.saveUiBackgroundImage')}
                                </Button>
                                <div className="flex items-center gap-2">
                                    <Label htmlFor="ui-background-blur">{t('admin.settings.uiBackgroundBlur')}</Label>
                                    <input
                                        id="ui-background-blur"
                                        type="range"
                                        min={0}
                                        max={100}
                                        value={normalizedBlur}
                                        onChange={(e) => setUiBackgroundBlurValue(e.target.value)}
                                    ...
                                    <Input
                                        ...
                                        max={100}
                                        className="w-20"
                                        value={uiBackgroundBlurValue}
                                        onChange={(e) => setUiBackgroundBlurValue(e.target.value)}
                                    />
                                    <span className="text-xs text-muted-foreground">{t('admin.settings.uiBackgroundBlurHint')}</span>
                                    <Button variant="outline" onClick={handleSaveUiBackgroundBlur} disabled={savingUiBackgroundBlur}>
                                        {savingUiBackgroundBlur ? t('common.processing') : t('admin.settings.saveUiBackgroundBlur')}
                                    </Button>
                                </div>
                            </div>
                        </div>
